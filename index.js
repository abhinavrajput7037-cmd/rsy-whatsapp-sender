...
app.get('/qr', (req, res) => {
    if (clientReady) {
        return res.send(`<html><body style="font-family:sans-serif;text-align:center;padding:40px;background:#0d0f1a;color:#fff;">
            ...
            <p style="color:#9ba3c8;">RSY Battle sender active hai.</p>
        </body></html>`);
    }
    if (!qrCodeData) {
        ...
        <body style="font-family:sans-serif;text-align:center;padding:40px;background:#0d0f1a;color:#fff;">
            ...
            <p style="color:#9ba3c8;">Page 4 sec mein auto-refresh hoga. Ruko thoda.</p>
        </body></html>`);
    }
    res.send(`<html><head><meta http-equiv="refresh" content="28"></head>
...
app.post('/send', async (req, res) => {
    ...
    try {
        await sock.sendMessage(formatNumber(mobile), {
            text: buildMessage({ method: method || 'redeem', amount, code, pin, name })
        });
...
app.post('/test', async (req, res) => {
    ...
    try {
        await sock.sendMessage(formatNumber(mobile), {
            text: `✅ *RSY Battle* — WhatsApp test successful! 🎮\nSab kuch kaam kar raha hai!`
        });
