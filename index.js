...
console.log('Chromium path:', chromiumPath || 'using default');
...
// ── Routes ──
app.get('/', (req, res) => {
    res.json({ service: 'RSY Battle WA Sender', status: clientReady ? 'connected' : 'disconnected', qr: !!qrCodeData });
});

app.get('/qr', (req, res) => {
    if (clientReady) {
        return res.send(`<html><body style="font-family:sans-serif;text-align:center;padding:40px;background:#0d0f1a;color:#fff;"><h2 style="color:#00c853;">✅ WhatsApp Connected!</h2><p style="color:#9ba3c8;">RSY Battle sender active hai.</p></body></html>`);
    }
    if (!qrCodeData) {
        return res.send(`<html><head><meta http-equiv="refresh" content="4"></head><body style="font-family:sans-serif;text-align:center;padding:40px;background:#0d0f1a;color:#fff;"><h2 style="color:#ffab00;">⏳ QR generate ho raha hai...</h2><p style="color:#9ba3c8;">Page 4 sec mein auto-refresh hoga. Ruko thoda.</p></body></html>`);
    }
...
app.post('/send', async (req, res) => {
    ...
    try {
        await client.sendMessage(formatNumber(mobile), buildMessage({ method: method || 'redeem', amount, code, pin, name }));
...
app.post('/test', async (req, res) => {
    ...
    if (!mobile) return res.status(400).json({ success: false, error: 'mobile required' });
    try {
        await client.sendMessage(formatNumber(mobile), `✅ *RSY Battle* — WhatsApp test successful! 🎮\nSab kuch kaam kar raha hai!`);
