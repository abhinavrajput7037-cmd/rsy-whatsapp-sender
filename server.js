...
async function initWhatsApp() {
    ...
    try {
        // ✅ Fixed: Correct Baileys v6 ESM import
        ...
        if (!makeWASocket) throw new Error('makeWASocket not found in baileys package');
...
if (RENDER_URL) {
    setInterval(async () => {
        try {
            const r = await fetch(RENDER_URL + '/status');
            const d = await r.json();
            console.log('🏓 Self-ping:', d.connected ? 'Connected' : 'Not connected');
